...
This document provides a technical deep-dive into the architectural patterns, component hierarchies, and data orchestration strategies that power the Portfolio Platform.

---

## 1. High-Level System Architecture

The interaction between the Client (Browser), Edge Network (Vercel), and Core Intelligence Services (Cerebras & Neon).
...
## 2. Component Hierarchy (React 19)

The application utilizes a Domain-Driven Component structure to maintain separation between the public portfolio and the high-security Admin Engine.
...
## 3. Data Flow: Autonomous AI Generation

The system has transitioned from a monolithic "Single-Shot" generation to a **Streaming-First** architecture.

### A. SSE Streaming Pipeline

**Problem**: Generating deep technical content (>3000 words) frequently exceeds Vercel's 30-second serverless timeout.  
**Solution**: **Server-Sent Events (SSE)**. The connection is held open while chunks of data are flushed immediately as they are generated by the Cerebras inference engine.
...
## 4. File Structure (Physical Layout)
...
## 5. Persistence Strategy: Neon PostgreSQL

The database is optimized for both transactional integrity (Auth/Messages) and full-text search (Blog).
...
## 6. Performance & Correctness Strategy
